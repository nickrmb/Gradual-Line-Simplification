...
public class SumMaxTotalDP extends DynamicProgramSimplifier {

	private SymmetricMatrix fromK;
	private SumMaxMerge smm = new SumMaxMerge();
	private Removal[] result;
	private SymmetricMatrix totalMax;

	@Override
	public void onSetup(int length) {
		fromK = new SymmetricMatrix(length, 0);
		totalMax = new SymmetricMatrix(length, 0);
	}

	@Override
	public void onMinFound(int i, int j, int minK) {
		fromK.setValue(i, j, minK);
		totalMax.setValue(i, j, Math.max(Math.max(totalMax.getValue(i, minK), totalMax.getValue(minK, j)), getShortcutDistance(i, j)));
	}

	@Override
	public double getError(int i, int j, int k, double error_ik, double error_kj, double shortcutDistance) {
		return error_ik + error_kj + Math.max(Math.max(totalMax.getValue(i, k), totalMax.getValue(k, j)), shortcutDistance);
	}
	...
	private Removal[] backtrack(SC sc) {
		...
		Removal r = new Removal(k, sc, totalMax.getValue(sc.i, sc.j));
